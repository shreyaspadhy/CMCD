...
def get_bin_counts(array_in: NpArray, num_bins_per_dim: int) -> NpArray:
...
def poisson_process_log_likelihood(
    latent_function: Array, bin_area: Array, flat_bin_counts: Array
) -> Array:
...
def get_latents_from_white(
    white: Array, const_mean: Array, cholesky_gram: Array
) -> Array:
...
def get_white_from_latents(
    latents: Array, const_mean: Array, cholesky_gram: Array
) -> Array:
    ...
    white = slinalg.solve_triangular(cholesky_gram, latents - const_mean, lower=True)
