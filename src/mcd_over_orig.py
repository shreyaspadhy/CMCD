...
def evolve_overdamped_orig(
    ...
    def evolve(aux, i):
        ...
        # Forward kernel
        fk_mean = z - params["eps"] * jax.grad(U)(
            z, beta
        ...
        # Backwards kernel
        if not use_sn:
            bk_mean = z_new - params["eps"] * jax.grad(U)(
                z_new, beta
            )  # Ignoring NN, assuming initialization, recovers method from Thin et al.
        else:
            bk_mean = (
                z_new
                - params["eps"] * jax.grad(U)(z_new, beta)
                + params["eps"] * apply_fun_sn(params["sn"], z_new, i)
